...
it('[useBalance] should load balance properly', async () => {
...
it('[useBalance] should update balance when address changes', async () => {
...
it('[useBalance] should return undefined for invalid address', async () => {
...
it('[useBalance] should return correct balance properties', async () => {
  const { result } = renderHook(() => useBalance(ALICE), { wrapper });
...
it('[useBalances] should load balances properly', async () => {
...
it('[useBalances] should update balances when addresses change', async () => {
...
it('[useBalances] should return correct balance properties for multiple addresses', async () => {
...
const deployContract = async () => {
  const contractAddress = await deployAndDeposit();
  console.log('Deployed contract contractAddress', contractAddress);
  const contract = new Contract(client, psp22 as any, contractAddress, { defaultCaller: ALICE });

  return { contract, contractAddress };
};

it('[useContract] get flipper value', async () => {
...
it('[useContract] should load balance properly', async () => {
...
it('[useContract] should update balance when address changes', async () => {
  const { contractAddress } = await deployContract();
...
it('[useContract] should return undefined for invalid address', async () => {
  const { contractAddress } = await deployContract();
...
it('[useDeployer] should load deployer properly', async () => {
...
it('[useDeployer] should update deployer when defaultCaller changes', async () => {
...
it('[useDeployerTx] should load deployerTx properly', async () => {
...
it('[useDeployerTx] should sign and send tx', async () => {
  const { result: resultDeployer } = renderHook(
    () => useDeployer<FlipperContractApi>(flipper as any, flipper.source.wasm!),
    { wrapper },
  );
